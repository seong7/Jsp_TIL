...
public class GuestBookMgr {
	...
	public void insertGuestBook(GuestBookBean bean) {
		...
		try {
			...
			////// now() : 현재값 (TABLE에서 COLUMN 타입이 DATE / DATETIME 이므로 타입에 맞는  now 값 insert 됨
	...
	//GuestBook Delete
	public void deleteGuestBook(GuestBookBean bean) {
		Connection con=null;
		PreparedStatement pstmt=null;
		String sql = "";
		try {
	...
	public void insertComment(CommentBean bean) {
		...
		try {
			...
			pstmt.setString(2, bean.getCid()); //댓글 작성자 id
			pstmt.setString(3, bean.getComment());
			pstmt.setString(4, bean.getCip());  // 댓글 작성자의 ip 
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			pool.freeConnection(con, pstmt);
	...
	//Comment Delete All (방명록 원글 삭제시 모든 하위 댓글 삭제)
...
//		GuestBookMgr gb = new GuestBookMgr();
//		gb.listGuestBook("aaa", "0");
//	}
}
